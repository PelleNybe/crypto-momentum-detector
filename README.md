...
<details>
<summary><b> Performance Improvements (Click to expand)</b></summary>
<br>
...
3.  ** Faster Backtest Execution**: Changed Pandas `iterrows` to `itertuples` in the core backtester loop, dramatically reducing overhead by ~14x.
...
4.  ** Realistic Backtesting Execution**: Backtester logic updated to incorporate proper Stop Loss and Take Profit evaluation natively checking daily highs and lows.
...
4.  ** Animated Terminal View**: Converted the static CLI table dump into a real-time updating dashboard using `rich.live`.
...
*   ** Dynamic Trailing Stop Loss (Chandelier Exit)**: Maximizes profit capture by utilizing an ATR-based Chandelier Exit that scales with volatility, replacing static stops.
