...
def process_ticker(ticker, period, interval, backtest=False, strategy_params=None, use_mtf=False):
    try:
        ...
        if backtest:
            ...
            bt = Backtester(df_with_signals, initial_balance=10000.0, fee_rate=0.001, slippage=0.0005, position_size=1.0)
            bt_results = bt.run()
...
def display_summary(results):
    ...
    top_macd = sorted(valid_results, key=lambda x: x['MACD'] - x['MACD_Signal'], reverse=True)[:3]
    ...
    console.print(Panel(summary_text, title="📊 Portfolio Summary", border_style="cyan"))
...
def display_action_recommendations(results):
    ...
    if strong_buys:
        action_text.append(f"[bold bright_green]🚀 STRONG BUY:[/bold bright_green] {len(strong_buys)} assets ({', '.join(strong_buys)}) show extreme bullish momentum.")
    if buys:
        action_text.append(f"[bold green]📈 BUY:[/bold green] {len(buys)} assets ({', '.join(buys)}) show favorable entry conditions.")
    if holds:
        action_text.append(f"[bold yellow]⏳ HOLD:[/bold yellow] {len(holds)} assets ({', '.join(holds)}) have neutral or conflicting momentum.")
    if sells:
        action_text.append(f"[bold red]📉 SELL:[/bold red] {len(sells)} assets ({', '.join(sells)}) show bearish momentum. Consider exiting.")
    if strong_sells:
        action_text.append(f"[bold bright_red]💥 STRONG SELL:[/bold bright_red] {len(strong_sells)} assets ({', '.join(strong_sells)}) show extreme bearish conditions.")

    if not action_text:
        action_text.append("No actionable signals at this time.")

    console.print(Panel("\n".join(action_text), title="🎯 Trading Action Recommendations", border_style="green"))
...
def display_backtest_results(results):
    ...
    console.print(Panel(Columns(bt_panels, equal=True, expand=True), title="[bold]⏪ Backtest Results (Historical)[/bold]", border_style="yellow"))

def main():
    parser = argparse.ArgumentParser(description="Crypto Momentum Detector CLI")
    parser.add_argument("--tickers", type=str, nargs="+", default=["BTC-USD", "ETH-USD"], help="List of cryptocurrency tickers to analyze")
    parser.add_argument("--period", type=str, default="6mo", help="Historical data period to fetch")
    parser.add_argument("--interval", type=str, default="1d", help="Interval of historical data")
    parser.add_argument("--export", type=str, default=None, help="File path to export the results as CSV")
    parser.add_argument("--backtest", action="store_true", help="Run a backtest on historical data and show performance")
    parser.add_argument("--use-mtf", action="store_true", help="Enable Multi-Timeframe (MTF) analysis for signal confirmation")
    ...
    mtf_status = "[bold green]Enabled[/bold green]" if use_mtf else "[bold red]Disabled[/bold red]"
    ...
    with Progress(
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {executor.submit(process_ticker, ticker, period, interval, backtest, strategy_params, use_mtf): ticker for ticker in tickers}
    ...
    for result in results:
        ...
        if result['error']:
            row_vals = [ticker, result['status'], "-", "-", "-", "-", "-", "[red]ERROR[/red]", "-"]
            if use_mtf:
                 row_vals.insert(7, "-")
            ...
            row_data = {
                ...
                "Signal": signal,
                "Stop_Loss": f"{sl:.2f}" if sl is not None and not math.isnan(sl) else "-",
                "Take_Profit": f"{tp:.2f}" if tp is not None and not math.isnan(tp) else "-"
            }
            if use_mtf:
                 row_data["HTF_Trend"] = "BULL" if latest.get('HTF_Trend', True) else "BEAR"
            if backtest:
                row_data["BT_Return_Pct"] = f"{latest['backtest'].get('Return %', 0):.2f}%"
                row_data["Max_DD_Pct"] = f"{latest['backtest'].get('Max Drawdown %', 0):.2f}%"
                row_data["Win_Rate_Pct"] = f"{latest['backtest'].get('Win Rate %', 0):.2f}%"
    ...
    if export_path:
        try:
            with open(export_path, 'w', newline='') as csvfile:
                fieldnames = ["Ticker", "Date", "Price (USD)", "RSI (14)", "MACD", "BB_Low", "BB_High"]
                if use_mtf:
                     fieldnames.append("HTF_Trend")
                fieldnames.extend(["Signal", "Stop_Loss", "Take_Profit"])
                if backtest:
                    fieldnames.extend(["BT_Return_Pct", "Max_DD_Pct", "Win_Rate_Pct", "Total_Trades"])
