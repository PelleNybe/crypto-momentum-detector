...
class SignalGenerator:
    def __init__(self, data: pd.DataFrame, rsi_buy_min: int = 40, rsi_buy_max: int = 70,
                 rsi_sell_min: int = 30, rsi_sell_max: int = 60, rsi_strong_buy: int = 30, rsi_strong_sell: int = 70, use_mtf: bool = False,
    ...
    def generate_signals(self) -> pd.DataFrame:
        ...
        is_bullish = (rsi > self.rsi_buy_min) & (rsi < self.rsi_buy_max) & (macd > macd_signal) & (close > ema_20) & htf_bullish
        is_bearish = (rsi < self.rsi_sell_max) & (rsi > self.rsi_sell_min) & (macd < macd_signal) & (close < ema_20) & htf_bearish

        strong_buy_cond = (rsi < self.rsi_strong_buy) & (macd > macd_signal) & (close < bb_low) & htf_bullish
        strong_sell_cond = (rsi > self.rsi_strong_sell) & (macd < macd_signal) & (close > bb_high) & htf_bearish
        ...
        df.loc[buy_mask, 'Stop_Loss'] = df.loc[buy_mask, 'Close'] - (df.loc[buy_mask, 'ATR_14'] * self.atr_sl_multiplier)
        df.loc[buy_mask, 'Take_Profit'] = df.loc[buy_mask, 'Close'] + (df.loc[buy_mask, 'ATR_14'] * self.atr_tp_multiplier)

        sell_mask = df['Signal'].isin(['SELL', 'STRONG SELL'])
        df.loc[sell_mask, 'Stop_Loss'] = df.loc[sell_mask, 'Close'] + (df.loc[sell_mask, 'ATR_14'] * self.atr_sl_multiplier)
        df.loc[sell_mask, 'Take_Profit'] = df.loc[sell_mask, 'Close'] - (df.loc[sell_mask, 'ATR_14'] * self.atr_tp_multiplier)
