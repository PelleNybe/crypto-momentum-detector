...
class WalkForwardOptimizer:
    ...
    def _generate_param_combinations(self) -> List[Dict[str, Any]]:
        ...
        for p in combinations:
            if p['rsi_buy_min'] < p['rsi_buy_max'] and p['rsi_sell_min'] < p['rsi_sell_max']:
    ...
    def run_optimization(self) -> Dict[str, Any]:
        ...
        for i, (train_index, test_index) in enumerate(tscv.split(self.data)):
            ...
            logger.info(f"Window {i+1}/{self.n_splits}: Training on {len(train_data)} rows, Testing on {len(test_data)} rows")
            ...
            logger.info(f"Best params for window {i+1}: {best_params} (Train Return: {best_fitness:.2f}%)")
        ...
        total_oos_return = ((current_oos_balance - self.initial_balance) / self.initial_balance) * 100
        ...
        if out_of_sample_equity_curve:
            ...
            if len(returns_arr) > 0 and returns_arr.std() != 0:
                oos_sharpe_ratio = (returns_arr.mean() / returns_arr.std()) * np.sqrt(365)
        ...
        oos_win_rate = (len(winning_trades) / total_trades * 100) if total_trades > 0 else 0.0

        gross_profit = sum([t.get('Return %', 0) / 100 * t.get('Entry Price', 0) for t in winning_trades]) # Approx for profit factor
        gross_loss = abs(sum([t.get('Return %', 0) / 100 * t.get('Entry Price', 0) for t in losing_trades]))
        oos_profit_factor = (gross_profit / gross_loss) if gross_loss > 0 else (float("inf") if gross_profit > 0 else 0.0)
